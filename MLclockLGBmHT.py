...
clf = MultiOutputRegressor(lgb.LGBMRegressor(n_jobs=-1))

# specify parameters and distributions to sample from
param_dist = {"estimator__boosting_type": ['gbdt','dart','goss'],
              "estimator__num_leaves": [2**4,2**5,2**6,2**7,2**8,2**9,2**10],
              "estimator__max_depth": [2,4,6,8,10,12,-1],
...
random_search = RandomizedSearchCV(clf, param_distributions=param_dist, scoring=my_scorer,
                                   n_iter=n_iter_search, cv=3, iid=False, verbose=10,random_state=25)
random_search.fit(x, y)
report(random_search.cv_results_)

reg = random_search.best_estimator_

pickle.dump(reg, open('reg.MLclockGBmHT2025_5000.sav','wb'))
